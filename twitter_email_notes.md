...
This should be all I need. But actually, quite painfully, this is written for Python 2.7. I spent a while working it out myself, but someone else did all the work for me on Stack. My sample version:
...
- I needed to add "send" to the scopes and rerun the permissions get
- The base64 conversion gave me a headache. Luckily I remembered that Python 2.7 didn't distinguish between b'' strings and literal '' strings. This matters because you need to send base64 a 'bytes-like' string, like b'hello world'. I didn't know, though, that you have to `decode` it afterwards.

Now, this still needs to be updated for the new `email` syntax, which uses `email.message.EmailMessage` instead of the deprecated `email.mime.text.MIMEtext`. It should work okay still.
